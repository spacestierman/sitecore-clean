  ...
  <script>
    ...
    function uploadToFTP() {
      ...
      $.ajax({
        ...
        error: function (err) {
          $("#additionalInfo").append("<br />Failed to upload to FTP.<br />Response: " + err.statusText + " (" + err.status + ")<br />" + err.responseText);
          uploadButton.attr("disabled", false);
          //saveButton.attr("disabled", false);
          //restartButton.attr("disabled", false);
          $('div#additionalInfo')[0].scrollTop = $('div#additionalInfo')[0].scrollHeight;
      ...
      $.ajax({
        ...
        success: function (msg) {
          ...
          if (code === 2) {
            ...
            $('#RestartButton').attr("disabled", false);
            $('#SpinnerContainer').css("visibility", "hidden");
          }
        ...
        error: function (err) {
          $("#additionalInfo").append("<br />Operation failed.<br />Response: " + err.statusText + " (" + err.status + ")<br />" + err.responseText);          
          $('#SavePackageButton').attr("disabled", false);
          $('#UploadToFTPButton').attr("disabled", false);
          $('#RestartButton').attr("disabled", false);
          $('#SpinnerContainer').css("visibility", "hidden");
    ...
    $(document).ready(function () {
      ...
      if ($('#progress').css('display') === "block" || $('#additionalInfo').css('display') === "block")  {
    ...
    <div class="wf-content">
      ...
      <div id="CollectorsContainer" runat="server" style="margin-top: 10px;" visible="False">
        ...
        <div style="margin: 10px 0px 0px 2px">
          <input type="checkbox" runat="server" id="CollectFromAllInstances" checked="true" />
          <label for="CollectFromAllInstances"><b>Collect packages from all instances that share Event Queue with the current Sitecore instance</b></label>
        </div>
      </div>
      ...
      <div style="text-align: center; display: none" runat="server" id="SpinnerContainer">
        <img src="/" style="width: 35px; height: 35px;" alt="Spinner" runat="server" id="spinnerImage"/>
      </div>
