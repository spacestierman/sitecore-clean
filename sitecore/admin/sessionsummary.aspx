...
<script runat="server">
    ...
    private static Owner[] GetData(object item)
    {
        ...
        if (provider != null)
        {
            ...
            return sessionsByOwner.GetOwners().Select(ToOwner).ToArray();
        }
        return null;
    }
    ...
    {
        var configNode = Factory.GetConfigNode(pathToConfigNode);
        ...
        foreach (var childNode in configNode.ChildNodes)
        {
            ...
            {
                var provider = Factory.CreateObject(string.Format("{0}/{1}", pathToConfigNode, sessionProviderNode.LocalName), true) as ISessionDataProvider;
    ...
    private static Owner ToOwner(OwnerInfo ownerInfo)
    {
        ...
        var result = new Owner
        {
            ...
            Status = status,
            ActiveSessions = ownerInfo.GetActiveSessions().Select(ToDistributedSession).ToArray()
        };
    ...
    {
        var isHealthy = sessionInfo.IsHealthy;

        var result = new DistributedSession
        {
    ...
    {
        var isHealthy = activityInfo.IsHealthy;

        var result = new DistributedActivity
        {
    ...
    private static KeyValuePair<string, string>[] ToStatistics(ReadOnlySerializationCollection collection)
    {
        ...
        {
            var keyName = (char.ToUpper(key.Value[0]) + key.Value.Substring(1)).Replace('_', ' ');
            var value = collection.Get(key) ?? string.Empty;
...
<form runat="server">
    <div>
        <asp:Repeater ID="sessionProvidersRepeater" runat="server">
            <ItemTemplate>
                <div style="font-size: x-large;"><%# DataBinder.Eval((KeyValuePair<string, ISessionDataProvider>)Container.DataItem, "Key") %></div>
                <hr/>
