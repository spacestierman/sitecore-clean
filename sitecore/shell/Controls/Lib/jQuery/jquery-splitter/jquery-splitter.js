  ...
  $.fn.splitter = function (args) {
    ...
    return this.each(function () {
      ...
      function endSplitMouse(evt) {
        ...
        panes.find('.hidden-dialog-overlay').remove();

        $(document).unbind("mousemove" + opts.eventNamespace + " mouseup" + opts.eventNamespace);

        args.onEndSplitMouse && args.onEndSplitMouse(pos);
      }
      ...
      var bar = $('<div />')
        ...
        .bind("mousedown" + opts.eventNamespace, function (evt) {
          ...
          panes.prepend('<div class="hidden-dialog-overlay" style="position:absolute;top:0;left:0;right:0;bottom:0;width:100%;height:100%;z-index:10000;"></div>');
      ...
      if (opts.resizeTo) {
        ...
        $(window).bind("resize" + opts.eventNamespace, function (e) {
          if (e.target == window) {
            var top = window == opts.resizeTo ? splitter.offset().top : 0,
      ...
      } else if (opts.resizeToWidth && $('.ie').length == 0) {
