Sitecore.PageModes.ChromeTypes.Placeholder = Sitecore.PageModes.ChromeTypes.ChromeType.extend( {
  ...
  _doUpdateRenderingProperties: function(renderingChrome, html) {
    renderingChrome.type.update({ html: html, updateChromeData: true });
    Sitecore.PageModes.ChromeManager.resetChromes();
  ...
  _loadRenderingFromUrl: function (url, callback) {
    this._frameLoadedUrl = url + "&rnd=" + Math.random();
    var options = {
      type: "POST",
      url: this._frameLoadedUrl,
      dataType: "html",
      data: {
        layout: JSON.stringify(Sitecore.LayoutDefinition.getLayoutDefinition())
      },
      ...
      success: this._frameLoaded.bind(this)
    };
  ...
  _frameLoaded: function (response) {
    var parser = new DOMParser();
    var doc = parser.parseFromString(response, "text/html");
    var renderingUniqueId = $sc.parseQuery(this._frameLoadedUrl)["sc_ruid"];
    ...
    if (renderingDomElement != null) {      
      ...
      var scLayout = doc.getElementById("scLayout");
      if (scLayout) {
        Sitecore.LayoutDefinition.setLayoutDefinition(scLayout.textContent);
      }
    }
    else
    {
      ...
      if (this._frameLoadedUrl.toLowerCase().indexOf("pagedesignererror.aspx") > -1) {
