...
var Flotr = {
    ...
    merge: function(src, dest) {
        ...
        for (i in src) {
            ...
            result[i] = (v &&
                    typeof(v) === 'object' &&
                    !(v.constructor === Array || v.constructor === RegExp) &&
                    !Object.isElement(v))
                ? Flotr.merge(v, dest[i])
                : result[i] = v;
        }
        return result;
    },
...
Flotr.defaultOptions = {
    colors: ['#4AC6FF', '#C0D800', '#CB4B4B', '#4DA74D', '#9440ED'],
...
Flotr.Graph = Class.create({
    ...
    */
    ...
    processColor: function(color, options) {
        ...
        if (!options.ctx) {
            ...
            return Flotr.Color.parse(Object.isArray(grad.colors[0]) ? grad.colors[0][1] : grad.colors[0])
                .scale(null, null, null, options.opacity)
                .toString();
        }
    ...
    registerPlugins: function() {
        ...
        for (name in Flotr.plugins) {
            ...
            for (c in plugin.callbacks) {
                ...
                this.el.stopObserving(c).observe(c, plugin.callbacks[c].bindAsEventListener(this));
            }
    ...
    */
    getTextDimensions: function(text, canvasStyle, HtmlStyle, className) {
        ...
        if (!this.options.HtmlText && this.textEnabled) {
            ...
            var dummyDiv = this.el.insert('<div style="position:absolute;top:-10000px;' +
                        HtmlStyle +
                        '" class="' +
                        className +
                        ' flotr-dummy-div">' +
        ...
        if (o.ticks) {
            ...
            if (o.mode == 'time') {
                ...
                for (i = min; i < max; i += axis.tickSize) {
                    ...
                    var decadeEnd = decadeStart *
                    ((o.base == Math.E) ? Math.exp(axis.tickSize) : Math.pow(o.base, axis.tickSize));
                ...
                for (i = 0; start + i * axis.tickSize <= axis.max; ++i) {
                    ...
                    if (o.minorTickFreq) {
                        for (var j = 0;
                            j < o.minorTickFreq && (i * axis.tickSize + j * minorTickSize) < axis.max;
                            ++j
                        ) {
    ...
    */
    calculateSpacing: function() {
        ...
        p.top += (options.grid.circular ? 0 : (x2.options.showLabels ? (x2.maxLabel.height + margin) : 0)) +
            (x2.options.title ? (x2.titleSize.height + margin) : 0) +
            this.subtitleHeight +
            this.titleHeight +
            maxOutset;
    ...
    */
    drawGrid: function() {
        ...
        if (o.grid.verticalLines ||
            o.grid.minorVerticalLines ||
            o.grid.horizontalLines ||
            o.grid.minorHorizontalLines) {
        ...
        if (o.grid.circular) {
            ...
            if (o.grid.horizontalLines) {
                ...
                for (var i = 0; i < a.ticks.length; ++i) {
                    ...
                    for (var j = 0; j <= sides; ++j) {
                        ctx[j == 0 ? 'moveTo' : 'lineTo'](Math.cos(j * coeff + angle) * radius * ratio,
            ...
            if (o.grid.backgroundColor != null) {
                ctx.fillStyle = this.processColor(o.grid.backgroundColor,
                { x1: 0, y1: 0, x2: this.plotWidth, y2: this.plotHeight });
    ...
    */
    drawOutline: function() {
        ...
        if (o.grid.circular) {
            ...
            */
            ctx.strokeStyle = o.grid.outlineColor;
            ctx.lineJoin = 'round';

            for (var i = 0; i <= sides; ++i) {
                ctx[i == 0 ? 'moveTo' : 'lineTo'](Math.cos(i * coeff + angle) * radius,
                    Math.sin(i * coeff + angle) * radius);
            }
        ...
        if (!options.HtmlText && this.textEnabled) {
            ...
            if (axis.options.showLabels) {
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    html.push(
                        ...
                        (this.plotOffset.top + this.plotHeight + options.grid.labelMargin),
                        'px;left:',
                        (this.plotOffset.left + axis.d2p(tick.v) - xBoxWidth / 2),
                        'px;width:',
                        xBoxWidth,
            ...
            if (axis.options.showLabels && axis.used) {
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    html.push(
                        '<div style="position:absolute;top:',
                        (this.plotOffset.top - options.grid.labelMargin - axis.maxLabel.height),
                        'px;left:',
                        (this.plotOffset.left + axis.d2p(tick.v) - xBoxWidth / 2),
                        'px;width:',
                        xBoxWidth,
            ...
            if (axis.options.showLabels) {
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    html.push(
                        '<div style="position:absolute;top:',
                        (this.plotOffset.top + axis.d2p(tick.v) - axis.maxLabel.height / 2),
                        'px;left:0;width:',
                        (this.plotOffset.left - options.grid.labelMargin),
                        'px;text-align:right;',
                        (axis.options.color ? ('color:' + axis.options.color + ';') : ''),
                        '" class="flotr-grid-label flotr-grid-label-y">',
                        tick.label,
                        '</div>'
                    );
                }
            ...
            if (axis.options.showLabels && axis.used) {
                ...
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    html.push(
                        ...
                        (this.plotOffset.top + axis.d2p(tick.v) - axis.maxLabel.height / 2),
                        'px;right:0;width:',
                        (this.plotOffset.right - options.grid.labelMargin),
                        'px;text-align:left;',
                        (axis.options.color ? ('color:' + axis.options.color + ';') : ''),
                        '" class="flotr-grid-label flotr-grid-label-y">',
                        tick.label,
        ...
        if (!options.HtmlText && this.textEnabled) {
            ...
            if (a.x.options.title && a.x.used)
                html.push(
                    ...
                    'px;width:',
                    this.plotWidth,
                    'px;text-align:center;" class="flotr-axis-title">',
                    a.x.options.title,
                    '</div>'
                );
            ...
            if (a.y.options.title && a.y.used)
                html.push(
                    ...
                    'px;left:0;text-align:right;" class="flotr-axis-title">',
                    a.y.options.title,
                    '</div>'
                );
            ...
            if (a.y2.options.title && a.y2.used)
                html.push(
                    '<div style="position:absolute;top:',
                    (this.plotOffset.top + this.plotHeight / 2 - a.y.titleSize.height / 2),
                    'px;right:0;text-align:right;" class="flotr-axis-title">',
                    a.y2.options.title,
                    '</div>'
                );
    ...
    insertLegend: function() {
        ...
        if (noLegendItems) {
            if (!options.HtmlText && this.textEnabled && !$(legend.container)) {
                ...
                // Legend box
                var color = this.processColor(options.legend.backgroundColor || 'rgb(240,240,240)',
                { opacity: options.legend.backgroundOpacity || 0.1 });
                ...
                for (i = 0; i < series.length; ++i) {
                    ...
                    var s = series[i],
                        ...
                        opacity = 'opacity:' +
                            ...
                            s.bars.fillOpacity * 100 +
                            ');',
                        color = 'background-color:' +
                            ((s.bars.show && s.bars.fillColor && s.bars.fill) ? s.bars.fillColor : s.color) +
                            ';';

                    fragments.push(
                        '<td class="flotr-legend-color-box">',
                        '<div style="border:1px solid ',
                        legend.labelBoxBorderColor,
                        ';padding:1px">',
                        '<div style="width:',
                        (boxWidth - 1),
                        'px;height:',
                ...
                if (fragments.length > 0) {
                    ...
                    if (options.legend.container != null) {
                        ...
                        var div = this.el.insert('<div class="flotr-legend" style="position:absolute;z-index:2;' +
                                pos +
                                '">' +
                                table +
                                '</div>')
                            .select('div.flotr-legend')[0];
                            ...
                            */
                            ...
                            if (c == null) {
                                var tmp = (options.grid.backgroundColor != null)
                                    ...
                                    : Flotr.Color.extract(div);
                                c = this.processColor(tmp, null, { opacity: 1 });
                            }
                            this.el.insert(
                                    '<div class="flotr-legend-bg" style="position:absolute;width:' +
                                    div.getWidth() +
                                    'px;height:' +
                                    div.getHeight() +
                                    'px;' +
    ...
    */
    mouseMoveHandler: function(event) {
        ...
        (this.options.mouse.track || this.series.any(function(s) { return s.mouse && s.mouse.track; })))
            this.hit(pos);
        //this.newHit(pos);
    ...
    */
    mouseDownHandler: function(event) {
        ...
        this.selectionInterval = setInterval(this.updateSelection.bindAsEventListener(this),
        ...
        if (s.mouse.lineColor != null) {
            ...
            if (!s.bars.show && !s.pie.show) {
                ...
                if (s.mouse.trackAll) {
                    ...
                    var bisection = n.sAngle < n.eAngle
                            ? (n.sAngle + n.eAngle) / 2
    ...
    */
    hit: function(mouse) {
            ...
            if (!options.pie.show) {
                for (i = 0; i < series.length; i++) {
                    ...
                    for (var j = 0, xpow, ypow; j < data.length; j++) {
                        ...
                        (s.bars.show &&
                            xdiff < s.bars.barWidth / 2 &&
                            ((y > 0 && my > 0 && my < y) || (y < 0 && my < 0 && my > y)))) {
                ...
                for (i = 0; i < series.length; i++) {
                    ...
                    if (c < radius + 10 &&
                    ((((a > 0 && sAngle < kat && eAngle > kat)) || (a < 0 && sAngle < kat2 && eAngle > kat2)) ||
                    (sAngle > eAngle &&
                    ((a > 0 && (sAngle < kat || eAngle > kat)) || (a < 0 && (sAngle < kat2 || eAngle > kat2)))))) {
                        n.x = x;
                        n.y = y;
        ...
        if (n.series &&
        (n.mouse && n.mouse.track && !prevHit || (prevHit /*&& (n.x != prevHit.x || n.y != prevHit.y)*/))) {
            ...
            if (!n.mouse.relative) { // absolute to the canvas
                ...
                if (!s.bars.show && !s.pie.show) {
                    ...
                    pos += 'left:' +
                        (m + plotOffset.left + n.xaxis.d2p(n.x - options.bars.barWidth / 2)) +
                        'px;right:auto;';
                } else {
                    var center = {
                            x: (this.plotWidth) / 2,
                        ...
                        radius = (Math.min(this.canvasWidth, this.canvasHeight) * s.pie.sizeRatio) / 2,
                        bisection = n.sAngle < n.eAngle
                            ? (n.sAngle + n.eAngle) / 2
                            : (n.sAngle + n.eAngle + 2 * Math.PI) / 2;

                    pos += 'bottom:' +
                        (m - plotOffset.top - center.y - Math.sin(bisection) * radius / 2 + +this.canvasHeight) +
                        'px;top:auto;';
                    pos += 'left:' +
                        (m + plotOffset.left + center.x + Math.cos(bisection) * radius / 2) +
                        'px;right:auto;';
                }
            }
            elStyle += pos;
...
Object.extend(Flotr.Color,
{
    ...
    parse: function(color) {
        ...
        if ((result = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(color)))
            return new Color(parseInt(result[1] + result[1], 16),
        ...
        // rgba(num%,num%,num%,num)
        if ((result =
            /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(color)))
            return new Color(parseFloat(result[1]) * 2.55,
...
Flotr.Date = {
    ...
    spec: [
        [1, "millisecond"], [20, "millisecond"], [50, "millisecond"], [100, "millisecond"], [200, "millisecond"],
        [500, "millisecond"],
...
Flotr.addType('lines',
{
    ...
    draw: function(series) {
        ...
        if (series.lines.fill) {
            ctx.fillStyle = this.processColor(series.lines.fillColor || series.color,
    ...
    */
    draw: function(series) {
        var ctx = this.ctx,
            ...
            lw = series.bars.lineWidth; //Math.min(series.bars.lineWidth, bw);
    ...
    plot: function(series, barWidth, offset, fill) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            if (series.bars.horizontal)
                ...
                var left = x - (series.bars.centered ? barWidth / 2 : 0),
                    right = x + barWidth - (series.bars.centered ? barWidth / 2 : 0),
                    bottom = stackOffset,
                    top = y + stackOffset;
    ...
    plotShadows: function(series, barWidth, offset) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            var height = Math
                .max(0, ya.d2p(bottom) - ya.d2p(top) - ((ya.d2p(bottom) + sw <= this.plotHeight) ? 0 : sw));

            ctx.fillStyle = 'rgba(0,0,0,0.05)';
            ctx.fillRect(Math.min(xa.d2p(left) + sw, this.plotWidth),
...
Flotr.addType('candles',
{
    ...
    plot: function(series, offset) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (series.candles.fill && !series.candles.barcharts) {
                ...
                ctx.fillRect(xa.d2p(left),
                    ya.d2p(top2) + offset,
                    xa.d2p(right) - xa.d2p(left),
                    ya.d2p(bottom2) - ya.d2p(top2));
            }
    ...
    plotShadows: function(series, offset) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            this.ctx.fillRect(Math.min(xa.d2p(left) + sw, this.plotWidth),
                Math.min(ya.d2p(top) + sw, this.plotWidth),
                width,
                height);
        }
...
Flotr.addType('markers',
{
    ...
    plot: function(x, y, label, options) {
        ...
        Flotr.drawText(ctx,
            ...
            left + margin,
            top + margin,
            { textBaseline: 'top', textAlign: 'left', size: options.fontSize });
    }
});
...
Flotr.addType('radar',
{
    ...
    plot: function(series, offset) {
        ...
        for (var i = 0; i < data.length; ++i) {
            ...
            ctx[i == 0 ? 'moveTo' : 'lineTo'](Math.cos(i * coeff + angle) * radius * ratio + offset,
                Math.sin(i * coeff + angle) * radius * ratio + offset);
        }
        ctx.closePath();
...
Flotr.addPlugin('spreadsheet',
{
    ...
    callbacks: {
        'flotr:afterconstruct': function() {
            ...
            ss.tabsContainer = new Element('div',
            { style: 'position:absolute;left:0px;width:' + this.canvasWidth + 'px' }).addClassName('flotr-tabs-group');
            ss.tabs = {
                graph: new Element('div', { style: 'float:left' }).addClassName('flotr-tab selected')
                    .update(this.options.spreadsheet.tabGraphLabel),
                data: new Element('div', { style: 'float:left' }).addClassName('flotr-tab')
    ...
    constructDataGrid: function() {
        ...
        var container = new Element('div',
        {
            style: 'left:0px;top:0px;width:' +
                this.canvasWidth +
                'px;height:' +
                (this.canvasHeight - this.spreadsheet.tabsContainer.getHeight() - 2) +
                'px;overflow:auto;'
    ...
    downloadCSV: function() {
        ...
        if (options.spreadsheet.decimalSeparator === options.spreadsheet.csvFileSeparator) {
            throw "The decimal separator is the same as the column separator (" +
                options.spreadsheet.decimalSeparator +
                ")";
        }
        ...
        if (Prototype.Browser.IE && !Flotr.isIE9) {
            csv = csv.replace(new RegExp(separator, 'g'), decodeURIComponent(separator))
                .replace(/%0A/g, '\n')
                .replace(/%0D/g, '\r');
