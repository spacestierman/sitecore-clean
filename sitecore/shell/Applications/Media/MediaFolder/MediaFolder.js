...
var SitecoreMediaUploader = Class.create({
  ...
  onUploadError: function(event) {
      console.warn("upload error. file: %s, message: %s", event.details["0"]._event.host.name, event.data);
      this.hadErrors = true;
    ...
    var row = this.yUploader.isFlash? $(event.originEvent.id): $(event.details[0].target.id);
      ...
      progress.down('img').remove();
      progress.setStyle({ backgroundImage: 'none' });
      progress.insert({ bottom: new Element("img", { src: '/-/icon/Office/16x16/delete.png.aspx', alt: event.data, title: event.data }) });
  ...
  updateUI: function() {
    ...
    /* in advanced mode only, we display the advanced options for the user */
    if (!this.simple) {
