define(["sitecore"], function (Sitecore) {
  var translationUtils = {
    keys: {
      ...
      Unfortunately_some_items_cannot_be_unlocked_because_they_are_locked_by_another_user: "Unfortunately some items cannot be unlocked, because they are locked by another user: ", // Server
    ...
    translateTextByServer: function (key, experienceEditor) {
      var result;

      var currentContext = experienceEditor.RibbonApp.getAppContext().app.currentContext;
      currentContext.value = key;
      experienceEditor.Web.postServerRequest("ExperienceEditor.TranslateText", currentContext, function (response) {
          ...
          experienceEditor.RibbonApp.getApp().handleResponseErrorMessage(response);
        }
      });
