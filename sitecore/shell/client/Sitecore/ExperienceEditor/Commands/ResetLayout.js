  ...
  Sitecore.Commands.ResetLayout =
  {
    ...
    execute: function (context) {
      if (context.app.canExecute("ExperienceEditor.IsEditAllVersionsTicked", context.currentContext) && ExperienceEditor.isInSharedLayout(context)) {
        ExperienceEditor.Dialogs.confirm(TranslationUtil.translateTextByServer(TranslationUtil.keys.Are_you_sure_you_want_to_reset_the_shared_layout_These_changes_will_affect_all_versions_of_this_page, ExperienceEditor), function (isOk) {
          if (!isOk) {
            return;
          }
