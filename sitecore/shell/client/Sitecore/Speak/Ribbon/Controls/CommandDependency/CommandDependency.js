  ...
  Sitecore.Factories.createBaseComponent({
    ...
    runDependenciesCanExecute: function(context) {
      ...
      for (var i = 0; i < dependantCommands.length; i++) {
          var controls = ExperienceEditor.CommandsUtil.getControlsByCommand(context.app, dependantCommands[i]);
        for (var j = 0; j < controls.length; j++) {
          ...
          var pageCodeScriptUrl = context.app.getPageCodeScriptFileUrl(control);
