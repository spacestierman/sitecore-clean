// Using jquery widgets is not the approach we would like to use for the future, 
// because it is hard to test.

define("ResizableColumns", ["underscore", "jquery", "jqueryui"], function (_, $) {

  $.widget("sc.resizableColumns", {
    options: {
      selectors: {
        headerColumn: "tr th.sc-table-head",
        column: "tr td.ventilate",
        handle: '.sc-rc-handle',
    ...
    _isResizeAllowed: function (difference) {
      ...
      return leftWidth >= this.options.minWidth && rightWidth >= this.options.minWidth;
    },
    ...
    _calculateWidth: function () {
      ...
      return _.map(this.$headers, _.bind(function (header) {
        return percentOf($(header).outerWidth(), this.tableWidth);
      }, this));
    },
  ...
  function indexOfElementInCollection(element, collection) {
    var index = -1;
