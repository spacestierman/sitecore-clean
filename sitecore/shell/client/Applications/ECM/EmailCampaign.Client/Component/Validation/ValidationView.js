	...
	function(sitecore, backbone) {
		return backbone.View.extend({
			...
			attachEvents: function() {
				this.model.on({
						'change:inputs': this.onChangeInputs,
						'validation:indicator:success': function(value, indicator) {
							var title = null;
							switch (value) {
							case 1:
								title = sitecore.Resources.Dictionary.translate('ECM.EmailValidation.Invalid');
								break;
							case 2:
								title = sitecore.Resources.Dictionary.translate('ECM.EmailValidation.Pending');
								break;
							case 3:
								title = sitecore.Resources.Dictionary.translate('ECM.EmailValidation.Valid');
								break;
							}
							indicator.viewModel.$el.attr('data-sc-indicator', value);
							indicator.viewModel.$el.attr('title', title);
						}
					},
					this);
			},
			...
			attachInputEvents: function(input, inputData) {
				input.on(inputData.validateEvent || this.model.get('inputValidateEvent'),
					_.bind(this.onInputChange, this, inputData),
					this);
			},
			...
			bindInputHighlight: function() {
				this.model.on({
						'validation:input:error': function(message, input) {
							if (input) {
								_.each($.type(input) !== 'array' ? [input] : input,
									_.bind(function(singleInput) {
