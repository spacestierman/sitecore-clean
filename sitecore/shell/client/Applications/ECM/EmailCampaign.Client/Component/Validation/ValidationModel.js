	...
	return backbone.Model.extend({
		...
		validate: function(value, validators, SpeakInput) {
			...
			for (var rule in validators) {
				...
				if (rule === 'remote') {
				    if (validator.requestParams && !validator.requestParams.async) {
                        return this.validateRemote(value, validator, SpeakInput);
                    } else {
				    ...
				    var validatorMethod = _.bind(validator.method, this),
				        validatorMessage = validator.message ||
				            ValidatorsService.messages[rule] ||
				            ValidatorsService.messages.default;
				    if (!validatorMethod(value, this.getValidatorParams(validator), SpeakInput, validatorMessage)) {
						if (!validator.silent) {
						    this.addError(validatorMessage,
								messageId,
								SpeakInput);
						}
						return false;
		...
		validateRemote: function(value, validator, SpeakInput) {
		    ...
		    var params = this.getValidatorParams(validator),
		        requestParams = validator.requestParams || {},
		        result;

			ServerRequest(_.extend({
			    data: _.extend({ value: value }, params),
				success: _.bind(function(jqXHR) {
					var messageId = this.getMessageId(this.getInputName(SpeakInput), 'remote'),
					    message = jqXHR.errorMessage || validator.message;
				        result = jqXHR.value;
						if (jqXHR.value === false || jqXHR.error) {
							this.addError(message, messageId, SpeakInput);
						} else {
							this.removeError(messageId, SpeakInput);
						}
						this.set('remotesInProgress', this.get('remotesInProgress') - 1);
		...
		validateSender: function (emailAddress, indicator) {
            if (!emailAddress || emailAddress.trim().length < 1) {
                this.trigger('validation:indicator:success', 1, indicator);
                return;
            }
