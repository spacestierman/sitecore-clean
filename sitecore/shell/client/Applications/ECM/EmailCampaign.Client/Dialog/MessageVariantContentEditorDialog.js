define([
    ...
    function(
        ...
        return DialogBase.extend({
            ...
            removeNestedScForm: function (parentFrame) {
                delete parentFrame.top.scIsDialogsInitialized;
                delete parentFrame.scForm;
                delete parentFrame.top.scForm;
                var $parentFrame = $(parentFrame);
                $parentFrame.off("load");
                $parentFrame.off("unload");
                $parentFrame.off("blur");

                _.each(parentFrame.frames, _.bind(function(frame) {
                    var contentWindow = frame.contentWindow;
                    this.removeNestedScForm(contentWindow);
                }, this));
            },

            updateFrame: function () {
                var modalDialogFrame = document.getElementById("jqueryModalDialogsFrame"),
                    frameWindow = this.Frame.viewModel.$el.get(0).contentWindow;

                this.removeNestedScForm(frameWindow);

                if (modalDialogFrame) {
                    $(modalDialogFrame).remove();
                }
