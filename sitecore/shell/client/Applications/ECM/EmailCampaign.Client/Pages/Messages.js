...
function(
	...
	var messagesPage = ListPageBase.extend({
		initialized: function() {
			...
			contextApp.SearchTextBox.viewModel.$el.on('keypress',
				function(event) {
			...
			contextApp.SearchTextBox.viewModel.$el.on('keyup',
				function(event) {
			...
			contextApp.on('action:search',
				function() {
					contextApp.SearchTextBox.viewModel.$el.focus();
				},
				contextApp);

			contextApp.on('action:reload',
				function() {
					ReloadDashboard();
				},
				contextApp);

			this.on('action:deletemessage',
				function() {
					this.deleteMessage(this.MessageList.get('selectedItem'),
						_.bind(function() {
			...
			contextApp.on('action:SearchItems',
				function() {
					var text = contextApp.SearchTextBox.viewModel.$el.find('input').val();
					contextApp.MessageListDataSource.set('search', text);
					contextApp.MessageList.set('selectedItem', null);
					contextApp.MessageList.viewModel.$el.find('.active').removeClass('active');
				},
				contextApp);


			// TODO: Implement action:import
			contextApp.on('action:import',
				function() {
			...
			this.MessageList.on('change:selectedItem',
				function() {
