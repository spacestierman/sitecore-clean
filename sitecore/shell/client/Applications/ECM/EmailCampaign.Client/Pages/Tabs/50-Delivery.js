...
function (
	...
	var deliveryTab = AppBase.extend({
		...
		initialized: function() {
            ...
            this.PersonalisationCheckBox.set('isChecked', this.MessageContext.get("usesPersonalisation"));
            this.UsePreferredLanguageEnableCheckBox.set('isChecked', this.MessageContext.get("usePreferredLanguage"));
			this.attachEventHandlers();
			this.initAbVariantSizeComboBox();
			this.initAutomaticIntervalsComboBox();
		    this.setupValidation();
		    this.fixScheduledStartDate();
        ...
        setupValidation: function() {
            GlobalValidationService.addInputs([
                {
                    input: this.NotificationEmailValueTextBox,
                    validators: {
                        emailListRemote: {
                            message: sitecore.Resources.Dictionary
                                .translate("ECM.MessagePage.TheFormatOfTheEmailAddressIsIncorrect")
                        }
                    },
                    validateEvent: "no-event"
                }
            ]);
        },
		...
		attachEventHandlers: function() {
			...
			this.on({
					'dispatch:send': function() {
						this.validateImpoListsAndDispatch('dispatch', false);
					},
					'dispatch:schedule': function() {
						this.validateImpoListsAndDispatch('dispatch', true);
					},
					'dispatch:starttest': function () { this.validateImpoListsAndDispatch('starttest', false); },
					'dispatch:scheduletest': function () { this.validateImpoListsAndDispatch('starttest', true); },
			...
			this.MessageContext.on({
					...
					'change:usesPersonalisation': function () {
					    this.PersonalisationCheckBox.set('isChecked', this.MessageContext.get("usesPersonalisation"));
                    },
					'change:usePreferredLanguage': function () {
                        this.UsePreferredLanguageEnableCheckBox.set('isChecked', this.MessageContext.get("usePreferredLanguage"));
					}
				},
				this);
		...
		attachScheduleEvents: function() {
			...
			this.PersonalisationCheckBox.on('change:isChecked', function () {
			    this.MessageContext.set("usesPersonalisation", this.PersonalisationCheckBox.get("isChecked"));
            }, this);

            this.UsePreferredLanguageEnableCheckBox.on('change:isChecked', function () {
                this.MessageContext.set("usePreferredLanguage", this.UsePreferredLanguageEnableCheckBox.get("isChecked"));
		    }, this);
	    ...
	    // SPEAK doesn't allow to disable past dates using date picker configuration. 
		//   So we forced to do it programmatically.
        fixScheduledStartDate: function() {
            if (this.ScheduledDateDatePicker && !this.ScheduledDateDatePicker.get("minDate")) {
                var widgetEl = this.ScheduledDateDatePicker.viewModel.$el;
                widgetEl.datepicker("option", "minDate", 0);
            }
        }, 
    ...
    checkListsBeingImported: function() {
        var defer = $.Deferred();
        ServerRequest(constants.ServerRequests.CHECK_RECIPIENT_LISTS, {
            type: "GET",
            data: {
                messageId: this.MessageContext.get("messageId")
            },
            success: function (response) {
                if (response.value === true) {
                    ...
                    DialogService.show("confirm", {
                        text: sitecore.Resources.Dictionary.translate("ECM.Message.OneOrMoreOfTheIncludedOrExcludedListsAreStillInTheProcess"),
                        on: {
                            ok: function() {
                                defer.resolve();
                            },
    ...
    validateImpoListsAndDispatch: function(actionName, isSchedule) {
        this.setControlsProperties(this.sendButtons, { isEnabled: false });
        this.checkListsBeingImported()
            .done(_.bind(this.dispatch, this, actionName, isSchedule))
		...
		dispatch: function (actionName, isSchedule) {
		    var isRecipientsValid = this.MessageContext.get('messageType') !== constants.MessageTypes.AUTOMATED
		            ? MessageValidationService.validateRecipients() : true,
		        validImages = MessageValidationService.validateImageResources(this.MessageContext.get('variants')),
		        validSubject = MessageValidationService.validateMessageVariantsSubject(this.MessageContext.get('variants'));

		    this.setControlsProperties(this.sendButtons, { isEnabled: false });

		    this.verifyMessage(actionName, isSchedule);
		},

		verifyMessage: function (actionName, isSchedule) {
