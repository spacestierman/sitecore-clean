...
], function (sitecore, TechnologyReport, Constants, MathHelper, ChartDataConversionService, ReportDataService) {
    var model = TechnologyReport.model.extend({
        defaults: {
            byMessageDataSource: null,
            dataSourceName: 'byDevice'
        },
        initialize: function() {
            if (!this.get('byMessageDataSource')) {
                this.set('byMessageDataSource', ReportDataService.getDataSource('byMessage'));
            }
            this._super();
        },
    ...
    var view = TechnologyReport.view.extend({
        ...
        updateValuePerVisit: function (type, scoreCardName) {
            ...
            if (mosts.valuePerVisit) {
                this.children[scoreCardName].set(
                    'value',
                    mosts.valuePerVisit.key + ' ' + new Number(MathHelper.divide(mosts.valuePerVisit.value.value, mosts.valuePerVisit.value.click, 2))
                );
                this.children[scoreCardName].set(
                    'description',
