(function() {
    var dependencies = (typeof window !== "undefined")
        ? ["sitecore", "/-/speak/v1/listmanager/cookieParser.js"]
        : [null, "./cookieParser"];
    define(dependencies,
        function(sitecore, cookieParser) {
            var pageIndexKey = "pageIndex",
                ieAccessDeniedMessage =
                    "At the moment, you cannot use or edit the list. Another user is using the list or its source. Please try again later.";
            ...
            return {
                ...
                showMoreCalled: function(parameters, callback) {
                    ...
                    var baseStructure = Array.prototype.filter.call(this.baseStructures,
                        function(e) {
                ...
                callController: function(parameters, urlQuery, onSuccessCallback, onErrorCallback, httpMethod) {
                    ...
                    this.callControllerDirectly(actionUrl, "", onSuccessCallback, onErrorCallback, null, httpMethod);
                },
                callControllerDirectly: function(actionUrl,
                    data,
                    onSuccessCallback,
                    onErrorCallback,
                    contentType,
                ...
                mergeListPages: function(parent, child) {
                    ...
                    for (var attr in parent) {
                        returnObject[attr] = parent[attr];
                    }
                    for (attr in child) {
                        returnObject[attr] = child[attr];
                    }
                    return returnObject;
                },
                ...
                onLoadHandler: function(iframe, onErrorCallback) {
                    ...
                    // Chrome case
                    else if (typeof iframe.contentDocument.body.innerText !== "undefined" &&
                        iframe.contentDocument.body.innerText !== "") {
                        onErrorCallback(JSON.parse(iframe.contentDocument.body.innerText).Message);
                    }
                    // Firefox case
                    else if (typeof iframe.contentDocument.body.textContent !== "undefined" &&
