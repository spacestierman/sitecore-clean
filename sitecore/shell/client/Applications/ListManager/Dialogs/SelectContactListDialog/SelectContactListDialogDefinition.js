(function() {
    ...
    var dependencies = (typeof window !== "undefined")
        ? [
            ...
            "/-/speak/v1/listmanager/commonListPagesDefinition.js"
        ]
        : [null, "../../commonPagesDefinition", {}];
    define(dependencies,
        function(sitecore, commonPagesDefinition, commonListPagesDefinition) {
            var self;
            ...
            var extensionObject = {
                    ...
                    Array.prototype.forEach.call(baseStructures,
                        function(baseStructure) {
                            if (current.isFirstInit) {
                                ...
                                baseStructure.control.off("change:selectedItem").on("change:selectedItem",
                                    function() {
                                        var realbaseStructure = Array.prototype.filter.call(current.baseStructures,
                                            function(e) {
                                    ...
                                    current);
                                baseStructure.dataSource.on("change:isBusy",
                                    function(dataSource, isBusy) {
                ...
                initializeSpecificControlsExtended: function() {
                    ...
                    //Sitecore trigger
                    current.on("app:loaded",
                        function() {
                            current.getSitecore().trigger("dialog:loaded", current);
                        });

                    current.getSitecore().on("select:contactlists:dialog:show",
                        function(callback, excludelists, currentListId, filter) {
                            current.showDialog({
                                callback: callback,
                ...
                initializeDataSourceExtended: function(baseStructure) {
                    baseStructure.dataSource.unsubscribeFromSelectList();
                },
                ...
                reloadListOfLists: function(baseStructure) {
                    ...
                    if (typeof baseStructure
                        .defaultPageSize !==
                        "undefined" &&
                        baseStructure.defaultPageSize !== null) {
                        currentPageSize = baseStructure.defaultPageSize;
                    }

                    if (typeof current.ExcludeLists !== "undefined" &&
                        current.ExcludeLists !== null &&
                        current.ExcludeLists.length !== 0) {
                        currentPageSize = currentPageSize + current.ExcludeLists.length;
                    }

                    if (typeof current.CurrentListId !== "undefined" &&
                ...
                initializeListOfLists: function(dataSource, isBusy, callback) {
                    if (isBusy === false && dataSource.isReady === true) {
                        var current = commonPagesDefinition.defaultIfValueIsUndefinedOrNull(self, this);
                        var baseStructure = Array.prototype.filter.call(current.baseStructures,
                            function(e) {
                        ...
                        if (typeof current.CurrentListId != "undefined" &&
                            ...
                            for (var i = 0; i < items.length; i++) {
                                if (items[i].Id == currentListId) {
                                    ...
                                    if (current.ExcludeLists != null &&
                                        $.inArray(items[i].Id, current.ExcludeLists) != -1) {
                                        items.splice(i--, 1);
                                    }
                                }
                            }
                            ...
                            for (var k = 0; k < items.length; k++) {
                                if (current
                                    .ExcludeLists !=
                                    null &&
                                    $.inArray(items[k].Id, current.ExcludeLists) != -1) {
                                    items.splice(k--, 1);
                                }
                            }
                        }
                ...
                showDialog: function(dialogParams) {
                    ...
                    if (typeof excludelists !== "undefined" &&
                        excludelists !== null &&
                        Object.prototype.toString.call(excludelists) === "[object Array]") {
                    ...
                    current.SelectContactListsDialog.viewModel.$el
                        .one('shown.bs.modal', _.bind(current.toggleProgressIndicator, current));
