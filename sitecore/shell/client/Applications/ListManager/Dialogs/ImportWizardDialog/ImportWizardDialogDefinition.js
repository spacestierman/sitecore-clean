    ...
    define(dependencies,
        function(sitecore, commonPagesDefinition) {
            var uploadTabId = "{63FD6D91-04E2-4CAE-80D3-6EA2362639B6}",
                ...
                importContactsFromMediaLibraryAndCreateListUrl =
                    "/sitecore/api/ssc/ListManagement/Import/{id}/ImportContactsFromMediaLibraryAndCreateList",
                importContactsFromMediaLibraryUrl =
                    "/sitecore/api/ssc/ListManagement/Import/{id}/ImportContactsFromMediaLibrary",
                contactListPagePattern = "/sitecore/client/Applications/List Manager/Taskpages/Contact list?id=",
                notificationKey =
                    "The import process may take a while. You can close this dialog and continue your work in Sitecore. You will not lose your work. You will be able to access your contacts when the import is complete.",
                theFileTypeIsInvalid = "The file type is invalid. Please select another file and try again.",
                emptyFileCannotBeImported =
                    "An empty file cannot be imported. Please select another file and try again.",
                onlyOneFileAtATime =
                    "You can upload only one file at a time. Please remove all other files and try again.",
                contactsNotImported = "The contacts have not been imported.",
                notificationTimeout = 10000,
                importWarningKey =
                    "Your contacts have been imported. However please note that they will only become available to view in the database after they have been indexed. This process may take some time.",
            ...
            return {
                initialized: function() {
                    ...
                    self.ButtonNext.viewModel.$el.parent('.pull-right')
                        .css({ 'position': 'absolute', 'clear': 'left', 'right': '11px', 'top': '35px' });
                    self.nextTimeout = null;
                    self.nextTimeoutDelay = 1500;

                    /*local triggers*/
                    self.on("import:wizard:dialog:button:previous:clicked",
                        function() { self.buttonPreviousClick(); },
                        self);
                    self.on("import:wizard:dialog:button:next:clicked", function() { self.buttonNextClick(); }, self);

                    self.on("import:wizard:dialog:button:finish:clicked",
                        function() { self.buttonFinishClick(); },
                        self);
                    ...
                    /* HACK FOR IE, required to make upload button work */
                    self.on("upload-fileAdded",
                        function(file) { self.UploaderInfo.viewModel.$el.find(".sc-uploaderInfo-row").focus(); },
                        self);

                    self.ImportMapTo1.on("import:mapto:select:changed",
                        function() { self.tabMapToSelectChanged(); },
                        self);

                    self.Uploader.on("uploadCompleted", function() { self.tabUploadOnFileUploadedComplete(); }, self);
                    self.ImportDataSource.on("change:isBusy",
                        function(response) { self.tabUploadOnFileUploadCompleteReadHeader(response); },
                        self);
                    ...
                    $("div.sc-dialogWindow.modal").on("hidden",
                        function() {
                            self.cleanUp();
                        });
                    ...
                    sitecore.on("import:wizard:dialog:show",
                ...
                cleanUp: function() {
                    ...
                    if (typeof self.UploadedFileInfo.ItemId !== "undefined" && self.UploadedFileInfo.ItemId !== null && !self.IsImporting) {
                        var removeMediaUrl = self.ImportControllerUrl +
                            "/" +
                            encodeURI(self.UploadedFileInfo.ItemId) +
                            "/RemoveMedia";
                ...
                updateButtonStatus: function() {
                    ...
                    current.ButtonPrevious.set("isEnabled",
                        current.ImportWizardTabControl.get(selectedTabProperty) != uploadTabId);
                ...
                importContacts: function() {
                    ...
                    var undefinedMapping = -1;
                    for (var index = 0; index < mapping.length; ++index) {
                        if (mapping[index].key != "" && mapping[index].value != undefinedMapping) {
                            if (mappingModel != "") {
                                mappingModel += '&';
                            }
                            var mappingKey = mapping[index].key;
                            try {
                                mappingKey = JSON.parse(mappingKey);
                                mappingKey.required = mapping[index].required;
                                mappingKey = JSON.stringify(mappingKey);
                            } catch (e) {
                            }
                    ...
                    commonPagesDefinition
                        .callControllerDirectly(actionUrl, mappingModel, successCallback, errorCallback);
                },
                showSummary: function(importSummary, timeout) {
                    ...
                    current.RequiredFieldsNotAvailableText
                        .set("text", importSummary.ContactsWithoutRequiredFieldsCount);
                ...
                showErrorSummary: function(timeout) {
                    ...
                    commonPagesDefinition.showError(this.StringDictionary.get(contactsNotImported),
                        current.ImportWizardDialogMessageBar);
                ...
                tabUploadOnFileAddedError: function(file) {
                    ...
                    case "ONLY_ONE_FILE_ALLOWED":
                        ...
                        break;
                    case "EMPTY_FILE":
                        message = this.StringDictionary.get(emptyFileCannotBeImported);
                        break;
                    default:
                    ...
                    current.ImportWizardDialogMessageBar.removeMessages("");
                    current.ImportWizardDialogMessageBar
                        .addMessage("error", { text: message, actions: [], closable: true });
                ...
                tabUploadOnFileUploadedComplete: function() {
                    ...
                    var getFileHeaderUrl = current.ImportControllerUrl +
                        "/" +
                        encodeURI(current.UploadedFileInfo.ItemId) +
                        "/GetContactFileHeaders";
                ...
                isTabEnabled: function(tabId) {
                    ...
                    $.map(current.ImportWizardTabControl.viewModel.$el.find("> ul li[data-tab-id]"),
                        function(tab) {
                            var $tab = $(tab);
                ...
                enableTab: function(tabId) {
                    ...
                    if (typeof current
                        .ImportWizardTabControl !==
                        "undefined" &&
                        current.ImportWizardTabControl !== null) {
                        $.map(current.ImportWizardTabControl.viewModel.$el.find("> ul li[data-tab-id]"),
                            function(tab) {
                                var $tab = $(tab);
                                ...
                                if (theTabId == tabId) {
                                    ...
                                    $tab.click(function(event) {
                                        current.ImportWizardTabControl.viewModel.onTabClicked
                                            .call(current.ImportWizardTabControl.viewModel, event);
                                        current.updateButtonStatus();
                                    });
                                }
                            });
