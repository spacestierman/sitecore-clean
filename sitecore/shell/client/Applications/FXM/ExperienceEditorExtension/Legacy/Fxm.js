define([
  ...
  var ctor = function () {
    ...
    this.focusChromePreviousTimeout = null;
    ...
    this.isCommandBar = function (node) {
      //scChromeDropDown class is for the popping-up sub-menus on the toolbar.
      return node.parents(".scChromeToolbar, .scChromeDropDown").length > 0;
    }
    ...
    this.focusChrome = function (node) {
      ...
      if (chrome) {
        if (this.focusChromePreviousTimeout) {
            clearTimeout(this.focusChromePreviousTimeout);
            this.focusChromePreviousTimeout = null;
        }

        var self = this;
        this.focusChromePreviousTimeout = setTimeout(function () {
          self.focusChromePreviousTimeout = null;
    ...
    this.onCapabilityChangedHandler = function () {
      ...
      if (this.isActive()) {
        //Remove all the previously assigned event handlers as subscription could happen more than once (e.g. during initial initialization).
