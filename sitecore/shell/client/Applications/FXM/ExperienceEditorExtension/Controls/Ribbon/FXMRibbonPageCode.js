  ...
  function (Sitecore, RibbonPageCode, _clientBeacon, $sc, _chromeControls, _clientActionInserter, _replacerInserter, TranslationUtil, ExperienceEditor) {
    var FxmPageCode = RibbonPageCode.extend({
      ...
      initialized: function () {
        ...
        var beaconNotLoadedNotification = TranslationUtil.translateTextByServer("Unable to locate JavaScript beacon on external website", ExperienceEditor);

        var self = this;
        var pipelineContext = ExperienceEditor.Common.cloneObject(this.currentContext);
          ...
          _clientBeacon.bindReady(function () {
            ...
            if (language && language.Name != self.currentContext.language) {
              ...
              var message = Sitecore.Helpers.string.format(
                TranslationUtil.translateTextByServer("This page is in {0}. You are currently creating items in {1}.", ExperienceEditor),
                language.Title,
                languageName);
              ...
              self.NotificationBar.addMessage("warning", {
                ...
                closable: false,
                actions: [{
                  text: Sitecore.Helpers.string.format(TranslationUtil.translateTextByServer("Change language to '{0}'", ExperienceEditor), language.Title),
