        ...
        sitecore.Factories.createBaseComponent({
            ...
            areKeysValid: function() {
                if (this.keysInvalidCharacter === undefined) {
                    ...
                    this.keysInvalidCharacter = experienceAnalytics
                        .findInvalidString(typeof keys === "string" ? keys.split("|") : keys,
                            experienceAnalytics.illegalCharacters);
                }
            ...
            validateProperties: function() {
                ...
                if (!this.areKeysValid()) {
                    this.showMessage("error",
                        errorTexts.NotAllowedCharacters,
                        { ILLEGAL_CHARACTER: this.keysInvalidCharacter });
                }
            ...
            getDataUrl: function(originalUrl, segments, keys, subsite, timeResolution) {
                var encodedKeys = this.pipeToComma(keys),
                    encodedSegments = this.pipeToComma(_.map(segments.split("|"),
                            function(segment) { return sitecore.Helpers.id.toShortId(segment); })
                        .join("|")),
