    ...
    function (sitecore) {
        var App = Backbone.Model.extend({
            ...
            getMetricId: function () {
                ...
                if (!metricId)
                    metricId = this.getSessionValue("metricId");

                return metricId;
            },

            getMetricName: function() {
                var metricDataField = this.get("metricData");
                if (!metricDataField)
                    metricDataField = this.getMetricFromUrl();

                return metricDataField;
            },

            setMetric: function (value) {
                var metricId = Boolean(value) ? value.itemId : null;
                this.setSessionValue("metricId", metricId);
                this.updateUrl({ metric: value.$displayName });
                this.set("metricId", metricId);
                this.set("metricName", value.$displayName);
